...
import { ArrowRight, BookOpen, Bookmark, ChevronLeft, ChevronRight, ExternalLink, FileText, Highlighter, Minus, Moon, Plus, Search, Sun, Trash2, X } from "lucide-react";
...
function pageCount(article: Article, isMobile: boolean) {
  if (article.viewer === "pdf") return article.pageCount ?? 1;
...
export default function Reader() {
  ...
  const blockPages = useMemo(() => active.viewer === "blocks" ? paginate(active.blocks ?? [], isMobile ? 2350 : 4600) : [], [active, isMobile]);
  const totalPages = active.viewer === "pdf" ? (active.pageCount ?? 1) : Math.max(1, blockPages.length);
  ...
  const captureSelection = () => {
    if (active.viewer !== "blocks") return;
  ...
  const pdfUrl = active.sourceUrl ? `${active.sourceUrl}#page=${currentPage + 1}&zoom=page-width&toolbar=1` : "";

  return <main className="reader-shell" style={{ "--reader-size": `${fontSize}px` } as React.CSSProperties}>
    <header className="reader-topbar"><button className="back-button" onClick={() => { setScreen("home"); setPending(null); }}><ChevronLeft size={20} /><span>Biblioteca</span></button><div className={`sync-status ${sync}`}><span />{sync === "saved" ? "Salvo" : sync === "saving" ? "Salvando" : sync === "error" ? "Sem sincronizar" : "Carregando"}</div><div className="reader-actions">{active.viewer === "blocks" && <><button className="icon-button" onClick={() => setFontSize((size) => Math.max(16, size - 1))} aria-label="Diminuir texto"><Minus size={18} /></button><button className="icon-button" onClick={() => setFontSize((size) => Math.min(24, size + 1))} aria-label="Aumentar texto"><Plus size={18} /></button></>}<button className="icon-button" onClick={toggleTheme} aria-label="Alternar tema">{theme === "light" ? <Moon size={18} /> : <Sun size={18} />}</button>{active.viewer === "blocks" && <button className="highlight-toggle" onClick={() => setShowHighlights(true)}><Highlighter size={17} /><span>Destaques</span><b>{highlights.length}</b></button>}</div></header>
    <div className="reading-progress"><span style={{ width: `${progressPercent}%` }} /></div>
    <div className={`reader-layout ${active.viewer === "pdf" ? "pdf-layout" : ""}`}><aside className="article-rail"><span className="rail-number">{String(articles.findIndex((item) => item.id === active.id) + 1).padStart(2, "0")}</span><span className="rail-line" /><span className="rail-label">{active.railLabel}<br />{active.year}</span></aside><article ref={articleRef} className={`article-page ${active.viewer === "pdf" ? "pdf-page" : ""}`} onMouseUp={captureSelection} onTouchEnd={() => window.setTimeout(captureSelection, 80)}>{currentPage === 0 && <header className="article-header"><span className="eyebrow">{active.viewer === "pdf" ? "Documento integral" : "Tradução para português"}</span><h1>{active.title}</h1><p className="authors">{active.authors}</p><div className="article-meta"><span>{active.journal}</span><span>{active.license}</span></div><p className="adaptation-note">{active.note}</p></header>}{active.viewer === "blocks" ? <div className="article-content">{blockPages[currentPage]?.map((block) => <Block key={block.id} block={block} highlights={highlights} />)}</div> : <section className="pdf-reader"><iframe key={`${active.id}-${currentPage}`} src={pdfUrl} title={`${active.title} — página ${currentPage + 1}`} /><div className="pdf-fallback"><FileText size={18} /><span>Página {currentPage + 1} de {totalPages}</span><a href={pdfUrl} target="_blank" rel="noreferrer">Abrir PDF original <ExternalLink size={14} /></a></div></section>}<nav className="page-navigation" aria-label="Navegação por páginas"><button disabled={currentPage === 0} onClick={() => { setCurrentPage((page) => Math.max(0, page - 1)); window.scrollTo(0, 0); }}><ChevronLeft size={18} />Anterior</button><span>Página <strong>{currentPage + 1}</strong> de {totalPages}<small>{active.viewer === "pdf" ? "documento original" : isMobile ? "versão celular" : "versão desktop"}</small></span><button disabled={currentPage === totalPages - 1} onClick={() => { setCurrentPage((page) => Math.min(totalPages - 1, page + 1)); window.scrollTo(0, 0); }}>Próxima<ChevronRight size={18} /></button></nav></article><aside className="side-notes">{active.viewer === "blocks" ? <><div className="side-notes-title"><Highlighter size={17} /><span>Destaques</span><b>{highlights.length}</b></div>{highlights.length ? highlights.slice(0, 6).map((item) => <button key={item.id} onClick={() => goToBlock(active.id, item.blockId)}>“{item.quote}”</button>) : <p>Selecione um trecho do texto para grifá-lo.</p>}</> : <><div className="side-notes-title"><FileText size={17} /><span>Fonte oficial</span></div><p>O documento é exibido integralmente, preservando páginas, tabelas e gráficos.</p><a className="source-link" href={active.doi} target="_blank" rel="noreferrer">Abrir na fonte <ExternalLink size={13} /></a></>}</aside></div>
